...
const VendorBookingsTable = () => {
  ...
  const optionsValue = [
    "notBooked", "booked", "onTrip", "notPicked",
    "canceled", "overDue", "tripCompleted",
  ];
  ...
  const fetchBookings = async () => {
    try {
      const res = await fetch("/api/vendor/vendorBookings", {
        ...
        credentials: "include",
        body: JSON.stringify({}), // vendor ID read from JWT
      });
  ...
  const getStatusClasses = (status) => {
    switch (status) {
      case "booked":
      case "onTrip": return "bg-emerald-50 text-emerald-700 border-emerald-200";
      case "canceled":
      case "overDue": return "bg-red-50 text-red-600 border-red-200";
      case "tripCompleted": return "bg-blue-50 text-blue-700 border-blue-200";
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto pb-16">
        {bookings.length > 0 ? (
          <p className="text-sm text-slate-600">Check out all bookings related to your vehicles.</p>
        ) : (
          ...
          <div className="space-y-4">
            {bookings.map((cur, idx) => {
              ...
              const vehicleImage = cur.vehicleId?.image?.[0]
                ...
                : "/vehicles/placeholder.png"; // optional fallback

              return (
                <div key={idx} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 md:p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    ...
                    <div className="md:col-span-2 flex flex-col justify-between">
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900 mb-1">Booking #{cur._id}</h3>
                        <p className="text-xs text-slate-500 mb-3">ID: {cur._id}</p>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between gap-6">
                        <div className="flex-1">
                          <div className="text-xs font-semibold text-slate-700 uppercase tracking-wide mb-2">Pick up</div>
                          <p className="text-sm text-slate-800 flex items-center gap-2 capitalize">
                            <CiLocationOn className="text-base" /> {cur.pickUpLocation}
                          </p>
                          <p className="text-xs text-slate-500 mt-1">
                            {pickupDate.getDate()}/{pickupDate.getMonth()+1}/{pickupDate.getFullYear()} - {pickupDate.getHours().toString().padStart(2,'0')}:{pickupDate.getMinutes().toString().padStart(2,'0')}
                          </p>
                        </div>

                        <div className="flex-1">
                          <div className="text-xs font-semibold text-slate-700 uppercase tracking-wide mb-2">Drop off</div>
                          <p className="text-sm text-slate-800 flex items-center gap-2 capitalize">
                            <CiLocationOn className="text-base" /> {cur.dropOffLocation}
                          </p>
                          <p className="text-xs text-slate-500 mt-1">
                            {dropoffDate.getDate()}/{dropoffDate.getMonth()+1}/{dropoffDate.getFullYear()} - {dropoffDate.getHours().toString().padStart(2,'0')}:{dropoffDate.getMinutes().toString().padStart(2,'0')}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex flex-wrap gap-3 items-center">
                          ...
                          <div className={`inline-flex items-center justify-center px-4 py-1.5 text-[11px] md:text-xs font-semibold capitalize rounded-full border ${getStatusClasses(cur.status)}`}>
                            {cur.status}
                          </div>
                        </div>
                      </div>
